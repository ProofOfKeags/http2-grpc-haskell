...
open ::
    ...
    HeaderList ->
    -- | Timeout in seconds. If nothing, assume infinite timeout. Only use this for streaming requests.
    Maybe Timeout ->
...
open conn authority extraheaders timeout encoding decoding call = do
    ...
    let timeoutHeader = case timeout of
            Nothing -> []
            Just n -> [(CI.original grpcTimeoutH, showTimeout n)]
    let request =
            [ (":method", "POST")
            , (":scheme", "https")
            , (":authority", authority)

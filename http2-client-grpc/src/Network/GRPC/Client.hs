...
type RawReply a
    = Either ErrorCode (CIHeaderList, Maybe CIHeaderList, Either String a)
...
waitReply rpc decoding stream flowControl =
    format
        .   fromStreamResult
        <$> waitStream stream flowControl throwOnPushPromise
  where
    ...
    let
        request =
            [ (":method"                , "POST")
                ...
                , ( CI.original grpcAcceptEncodingH
                  , mconcat
                      [ grpcAcceptEncodingHVdefault
                      , ","
                      , grpcCompressionHV decompress
                      ]
                  )
        ...
        in
            do
                ...
                _waitEvent stream >>= \case
                    ...
                    _ -> lift $ throwIO (InvalidState "no headers")
  where
    handleAllChunks decoding v1 hdrs decode dat exitLoop =
        case pushChunk decode dat of
            ...
            Done _ _ (Left err) ->
                lift $ throwIO
                    (StreamReplyDecodingError $ "done-error: " ++ err)
            Fail _ _ err -> lift
                $ throwIO (StreamReplyDecodingError $ "fail-error: " ++ err)
            partial@(Partial _) -> exitLoop v1 partial hdrs
...
streamRequest rpc v0 handler =
    RPCCall rpc $ \conn stream isfc streamFlowControl encoding decoding ->
        let
...
sendSingleMessage
    ...
    -> OutgoingFlowControl
    -> ClientIO ()
sendSingleMessage rpc msg encoding flagMod conn connectionFlowControl stream streamFlowControl
    = do
        let compress = _getEncodingCompression encoding
        let goUpload dat = do
...
singleRequest rpc msg =
    RPCCall rpc $ \conn stream isfc osfc encoding decoding -> do
        let ocfc = _outgoingFlowControl conn
...
steppedBiDiStream
  ...
  -- ^ The program.
    -> RPCCall r a
steppedBiDiStream rpc v0 handler =
    RPCCall rpc $ \conn stream isfc streamFlowControl encoding decoding ->
        let
            ocfc       = _outgoingFlowControl conn
            decompress = _getDecodingCompression decoding
                    ...
                    StreamDataEvent _ dat -> do
                        ...
                        case pushChunk decode dat of
                            Done unusedDat _ (Right val) -> do
                                ...
                                handler v2 >>= goStep
                                    jhdrs
                                    (pushChunk newDecoder unusedDat)
                            Done _ _ (Left err) ->
                                lift
                                    $  throwIO
                                    $  InvalidParse
            ...
            goStep Nothing decode unchanged = _waitEvent stream >>= \case
                StreamHeadersEvent _ hdrs ->
...
generalHandler rpc v0 handle w0 next =
  ...
  where
    go conn stream isfc osfc encoding decoding = concurrently
        (incomingLoop Nothing newDecoder v0)
        (outGoingLoop w0)
      where
        ocfc       = _outgoingFlowControl conn
        newDecoder = decodeOutput rpc decompress
            ...
            case event of
                ...
                SendMessage doCompress msg -> do
                    ...
                    sendSingleMessage rpc
                                      ...
                                      osfc
                    outGoingLoop v2
        incomingLoop Nothing decode v1 = _waitEvent stream >>= \case
            StreamHeadersEvent _ hdrs ->
                handle v1 (Headers hdrs) >>= incomingLoop (Just hdrs) decode
            _ -> handle v1 (Invalid $ toException $ InvalidState "no headers")
        incomingLoop jhdrs decode v1 = _waitEvent stream >>= \case
            StreamHeadersEvent _ hdrs -> handle v1 (Trailers hdrs)
            StreamDataEvent    _ dat  -> do
                liftIO $ _addCredit isfc (ByteString.length dat)
                _ <- liftIO $ _consumeCredit isfc (ByteString.length dat)
                _ <- _updateWindow isfc
                case pushChunk decode dat of
                    Done unusedDat _ (Right val) ->
                        handle v1 (RecvMessage val)
                            >>= incomingLoop
                                    jhdrs
                                    (pushChunk newDecoder unusedDat)
                    partial@(Partial _) -> incomingLoop jhdrs partial v1
                    Done _ _ (Left err) -> handle
                        v1
                        (  Invalid
                        $  toException
                        $  InvalidParse
